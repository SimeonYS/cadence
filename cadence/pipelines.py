...
class CadencePipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cadence`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cadence WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cadence (date, title, link, content)"
